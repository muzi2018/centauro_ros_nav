#include <centauro_ros_nav/backward_recovery.h>
...
PLUGINLIB_EXPORT_CLASS(backward_recovery::BackwardRecovery, nav_core::RecoveryBehavior)

namespace backward_recovery
{
BackwardRecovery::BackwardRecovery(): local_costmap_(NULL), initialized_(false), world_model_(NULL)
{
}

void BackwardRecovery::initialize(std::string name, tf2_ros::Buffer*,
...
BackwardRecovery::~BackwardRecovery()
{
  delete world_model_;
}

void BackwardRecovery::runBehavior()
{
  if (!initialized_)
  {
  ...
  ROS_WARN("Backward recovery behavior started.");
  ...
  while (n.ok() && dist < max_dist)
    {
      ...
      {
        ROS_ERROR("Backward recovery can't move back because there is a potential collision. Cost: %.2f",
                  footprint_cost);
        return;
      }
